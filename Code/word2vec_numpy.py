...
import sys
import string
from rnn_c.brown import get_sentences_with_word2idx_limit_vocab as get_brown
import config

sys.path.append(os.path.abspath('..'))
# from util import find_analogie
...
#%%
def get_wiki():
    # Input: wiki english artcles
    # Process: (1) count the words and (2) pick most common words
    # Output: sentencess, word2idx

    vocab_size = 20000

    # The 'glob' module finds all the pathnames matching a specified pattern according to the rules used by the Unix
    # shell, although results are returned in arbitrary order. No tilde expansion is done, but *, ?, and character
    # ranges expressed with [] will be correctly matched.

    files = glob(config.WIKIPEDIA_DATA + '/enwiki*.txt')
    all_word_counts = {}
    for f in files:
        for line in open(f):

            # If a line starts with special character it means its a meta data and we skip those lines
    ...
    # All low freq. words are assigned to UNK
    # We keep most freq. words
...
def train_model(savedir):
    ...
    # learning rate decay: (man - min)/# epochs
    ...
    # for sub-sampling each sentence
    threshold = 1e-5
    ...
    if not os.path.exists(savedir):
        os.mkdir(savedir)
    # saving both word2idx and weights to
    # know which slice of weight matrx belong to which word in our vocab
...
def get_negative_sampling_distribution(sentences, vocab_size):
    ...
    # make sure its a valid  prob distribution
    assert (np.all(p_neg > 0))
    return p_neg


def get_context(pos, sentence, window_size):
    # get the context given the middle word
    # input:a sentence of the form: x x x x c c c pos c c c x x x x
    # output: the context word indices: c c c c c c

    # starting point should be either (middle word - context size) or (zero)
    # the ending point can be either the (middle words position + context size) or (length of the sentense)
...
def sgd(input_, targets, label, learning_rate, W, V):
    ...
    # sometimes the weight probabilities become
    # too close to 0 or 1 which endsup breaking
    # the cost function so we add a small number (1e-10)
    # in the log function to mitigate this
