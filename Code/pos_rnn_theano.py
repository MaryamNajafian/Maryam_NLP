...
class RNN:
    def __init__(self, D, hidden_layer_sizes, V, K):
        self.hidden_layer_sizes = hidden_layer_sizes  # multiple recurrent units
        self.D = D
        self.V = V
        self.K = K
    ...
    def fit(self, X, Y, learning_rate=1e-4, mu=0.99, epochs=30, show_fig=True, activation=T.nnet.relu,
        ...
        # we don't normalize word embeddings: normalize=False
        D = self.D  # inputs are of size D
        V = self.V  # vocab size
        ...
        cost = -T.mean(T.log(py_x[T.arange(thY.shape[0]), thY]))  # softmax cross entropy
        grads = T.grad(cost, self.params)
        dparams = [theano.shared(p.get_value() * 0) for p in self.params]  # dparams is needed for momentum
