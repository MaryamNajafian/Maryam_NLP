...
"""
calculating Word vectors using PMI 
PMI: Pairwise Mutual Information word2vec as matrix factorization
PMI is based on both W2V and Glove methods
"""
...
# save a slot for <UNK>
V = min(V, len(all_word_counts)) #i ncase vocab size is smaller that word cound
...
if not os.path.exists('pmi_counts_%s.npz' % V):
  # initialize aparse matrix for word counts of size V by V
  ...
  for f in files:
    for line in open(f):
      # don't count sentences that are headers, structured data, lists, etc...
      if line and line[0] not in ('[', '*', '-', '|', '=', '{', '}'):
        line_as_idx = []
        for word in remove_punctuation(line).lower().split():
          if word in word2idx:
...
c_probs = c_counts / c_counts.sum() #unigram probability
...
# we added +1 in the log so incase there is a 0 entery we dont endup with NAN
logX = pmi.log1p() # would be logX = np.log(pmi.A + 1) in numpy
print("type(logX):", type(logX))
logX[logX < 0] = 0 #set any negative values to 0
...
for epoch in range(10):
  ...
  W = np.linalg.solve(matrix, vector).T # solve is applieed to the (matrix,matrix) combination here
