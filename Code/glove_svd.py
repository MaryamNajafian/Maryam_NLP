...
Apply Singular Value Decomposition (SVD) to the co-occurance matrix
We are NOT use
            bias terms or 
            regularization or 
            Gradient Decent or
...
We use SVD to find  W and U with scikit-learn's TruncatedSVD witth D variables
"""
...
class Glove:
    ...
    def fit(self, sentences, cc_matrix=None):
        ...
        model = TruncatedSVD(n_components=D) # D components: D latent variables
        Z = model.fit_transform(logX - mu) # subtract mu from logX because SVD assumes your data is centered
        S = np.diag(model.explained_variance_)
        Sinv = np.linalg.inv(S) # calculate S inverse
        self.W = Z.dot(Sinv)
