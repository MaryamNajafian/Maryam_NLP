...
INPUTS: we have 4 sequences we need to loop through simultaneously 
    * words list: word index of the current word if current node represent a word 
    * left_children: location of left child of the current node
    * right_children: location of right child of the current node
    * labels: positive: (encoded with: 1, rating: 3,4), neutral: (encoded with: -1, rating: 2), negative: (encoded with: 0, rating: 0,1) we can ignore neutral labels to avoid class imbalance
        * we can train the model using only label at root node
        * we can train the model using labels at all nodes

PROCESS:
    * If we are building a plain recursive network we do a linear transformation
    * If we have an RNTN then we do a quadratic transformation
    * we use SGD : one sample at a time
    
OUTPUT: model, and classification score
    * when it comes to calculating the accuracy we only care about the prediction at the root node, despite the fact that we maybe training on the inner nodes
...
class RecursiveNN:
    def __init__(self, V, D, K, activation=tf.tanh):
        self.V = V # vocab size
        self.D = D # embedding dimension (output dimension of every tree node)
        self.K = K # number of output classes
        self.f = activation # activation function

    def fit(self, trees, test_trees, reg=1e-3, epochs=8, train_inner_nodes=False):
        """
        trees have already been transformed to sequences
        inputs:
             train trees,
             test trees,
             regularization parameter,
             number of epochs,
             True/False flag for whether or not we are going to train using inner nodes
        """
        ...
        We = init_weight(V, D) # word embedding
        ...
        # we initialize the place holders that represent same data structure as how our trees are stored
        ...
        words = tf.placeholder(tf.int32, shape=(None,), name='words') # words list
        left_children = tf.placeholder(tf.int32, shape=(None,), name='left_children') # left children
        right_children = tf.placeholder(tf.int32, shape=(None,), name='right_children') # right children
        labels = tf.placeholder(tf.int32, shape=(None,), name='labels') # labels

        # save as instance variable for later
        self.words = words
        ...
        def recurrence(hiddens, n):
            w = words[n] # get the current word

            # check if the its leaf node or not:any non-word will have index -1

            # h_n: value of hidden value at index n
            h_n = tf.cond(
                w >= 0,
        ...
        if train_inner_nodes:
            # filter out -1s (neutral comments)
        ...
        else: #if train_inner_nodes=False

            cost_op = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(
                    # because we are using post order traversal the root node appears last at index -1
        ...
        for i in range(epochs):
            # we are using SGD so we are looking one sample at a time
            t0 = datetime.now()
...
def add_idx_to_tree(tree, current_idx):
    ...
    # add_idx_to_tree: is going to recursively number each node so we can use those indexes later when we assign left and right children
    if tree is None:
        return current_idx
...
def tree2list(tree, parent_idx, is_binary=False):
    # takes in a tree object and return:
    # 4 lists are generated using post order traversal (checking left children recursively, then right recursively, finally the root node
    # 4 lists: words, left_child, right_child, labels
    if tree is None:
        return [], [], [], []
    ...
    if is_binary:
        if tree.label > 2:
            ...
            label = -1 # neutral sentiment, we will eventually filter these out
    else:
        label = tree.label
