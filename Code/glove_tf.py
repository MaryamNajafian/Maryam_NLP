...
Since TF has automatic differentiation (unlike numpy)
We are using Gradient Decent instead of ALS (Alternating  Least Squares)
TF  has  its own optimizers so we are not not limited to plain Gradient Decent
"""
...
class Glove:
    def __init__(self, D, V, context_sz):
        self.D = D #embeding dimention
        self.V = V #vocab size
        self.context_sz = context_sz #context size
    ...
    def fit(self, sentences, cc_matrix=None, learning_rate=1e-4, reg=0.1, xmax=100, alpha=0.75, epochs=10):
        ...
        if not os.path.exists(cc_matrix):
            ...
            for sentence in sentences:
                ...
                n = len(sentence)
                for i in range(n):
                    # it is easier to loop over the positions
                    # in the sentence rather than the indexes
                    # because we need the position in order to
                    # calculate the left and right bounds of
                    # the context window
                    # i refers to the position in sentence
                    # and not the actual word index!!!
                    # wi is the corresponding word to position i in sentence
                    # j is not the word index!!!
                    # i just points to which element of the sequence (sentence) we're looking at
                    wi = sentence[i]

                    start = max(0, i - self.context_sz) # start is 0 or (i - self.context_sz)
                    end = min(n, i + self.context_sz)# end is length of the sentence (n) or (i + self.context_sz)
                    ...
                    if i - self.context_sz < 0:
                        # if the left side of the context goes out of bounds
                        # we want the count X to reflect the distance between
                        # the input word and the context word
                        # so we have a (1/1+distance) drop-off for each word in the context
                        points = 1.0 / (i + 1)
                        X[wi,0] += points
                        X[0,wi] += points

                    if i + self.context_sz > n:
                        # if the right side of the context goes out of bounds
                        points = 1.0 / (n - i)
                        X[wi,1] += points
                        X[1,wi] += points

                    # we loop through the words on the left side
                    # add the counts for words on left side context
                    ...
                    # we loop through the words on the right side
            ...
            # save the co-occurence matrix cc_matrix because it a long time to create
            np.save(cc_matrix, X)
        else:
            X = np.load(cc_matrix)
        ...
        # we need to weight the co-occurence matrix cc_matrix with function fX
        fX = np.zeros((V, V))
        ...
        # target is in logX format cause data has long tail disttribution(large non-zero values)
        # we add +1 to log to avoid NAN for log(X=0)
        logX = np.log(X + 1)
        ...
        # the difference between our prediction and actual value logX
        delta = tf.matmul(tfW, tf.transpose(tfU)) + tfb + tfc + mu - tfLogX
        # cost is the squared error but with every entry weighted by fx
        cost = tf.reduce_sum(tffX * delta * delta)
        regularized_cost = cost
        for param in (tfW, tfU):
            # you can use a regularization if you want
            regularized_cost += reg*tf.reduce_sum(param * param)


        # we create an optimizer to minimize the cost
        # here we chose MomentumOptimizer
